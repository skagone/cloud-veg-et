...
def _make_chip_poly(ul_lat, ul_lon, increment, xchip_increment, ychip_increment):
    coord_list = []
    ul_lat = int(ul_lat)
    ul_lon = int(ul_lon)
...
class GridMeister:
    ...
    def __init__(self, tile_name, raster_extent, x_ras_res, y_ras_res):

        self.tile_name = tile_name
        self.chip_increment = 2
        self.extent = raster_extent
        self.width = x_ras_res
        self.height = y_ras_res
        self.xchip_increment = None
        self.ychip_increment = None
            
    def dyn_chip_list(self, max_pixels=1000000):
        CHIP_LIST = []
        box={'left': self.extent[0], 'bottom': self.extent[0] , 
             'right': self.extent[0], 'top': self.extent[0]}
        starting_lat = box['top']
        ending_lat = box['bottom']
        print(starting_lat, ending_lat)
        
        starting_lon = box['right']
        ending_lon = box['left']
        print(starting_lon, ending_lon)
        

        # loop
        lat_cols = abs((starting_lat - ending_lat) / self.width)
        lon_rows = abs((starting_lon - ending_lon) / self.height)
        chip_pixels = lat_cols * lon_rows
        count = 1
        while chip_pixels > max_pixels:
            count += 1
            lat_cols = abs(((starting_lat - ending_lat)/count) / self.width)
            lon_rows = abs(((starting_lon - ending_lon)/count) / self.height)
            self.xchip_increment = ((starting_lat - ending_lat)/count)
            self.ychip_increment = ((starting_lon - ending_lon)/count)
            chip_pixels = lat_cols * lon_rows
        ...
        while lat > ending_lat:
            ...
            while lon < ending_lon:
                ...
                lon = lon + self.ychip_increment

            lat = lat - self.xchip_increment
    ...
    def create_chip_shp(self, ul_lat, ul_lon):
        self.aoi_dir = './AOI'
        print(ul_lat,ul_lon)
