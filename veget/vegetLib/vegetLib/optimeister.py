...
def _make_npy_cache_name(geotiff_fullname):
    print(geotiff_fullname)
    new_name = '/'.join(geotiff_fullname.split('/')[2:]) + '.npy'
    return new_name
...
class OptiMeister:
    ...
    def _cache_npy(self, warpfile,data):
        self.log.info('Cache this file {}'.format(warpfile))
        new_name = _make_npy_cache_name(warpfile)
        self.log.info('NPY Name {}'.format(new_name))
